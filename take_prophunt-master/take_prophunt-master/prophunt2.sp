		...
		if(IsPlayerAlive(client))
		{
			...
			if(GetClientTeam(client) == TEAM_PROP)
			{
				...
				if((buttons & IN_ATTACK2) == IN_ATTACK2 && !g_HoldingRMB[client])
				{
					...
					if(g_First[client])
					{
						PrintHintText(client, "3인칭"); //77
						SwitchView(client, true, false);
					}
					else
					{
						PrintHintText(client, "1인칭");
		...
		for(new client2=1; client2 <= MaxClients; client2++)
		{
			if(IsClientInGame(client2) && !IsFakeClient(client2) && IsPlayerAlive(client2))
			{
				if(GetClientTeam(client2) == TEAM_PROP) //77
				{
					...
					ShowHudText(client2, -1, "죽일 때마다 체력이 풀로 채워집니다.");
						
					SetHudTextParams(-1.0, 0.32, 10.0, 0, 204, 255, 255, 2, 1.0, 0.05, 0.5);
					ShowHudText(client2, -1, "근접무기는 황금렌치로 변경되며 데미지가 한방입니다.");
				...
				if(GetClientTeam(client2) == TEAM_HUNTER)
				{
					...
					SetHudTextParams(-1.0, 0.22, 10.0, 255, 234, 255, 0, 2, 1.0, 0.05, 0.5);
					ShowHudText(client2, -1, "레드팀 마지막 1인이 무기를 얻었습니다.");
						
					SetHudTextParams(-1.0, 0.27, 10.0, 249, 255, 61, 255, 2, 1.0, 0.05, 0.5);
					ShowHudText(client2, -1, "조심하세요!");
...
public Action:Timer_Locked(Handle:timer, any:entity)
{
	for(new client=1; client <= MaxClients; client++)
	{
		if(g_RotLocked[client] && IsClientInGame(client) && !IsFakeClient(client) && IsPlayerAlive(client) && GetClientTeam(client) == TEAM_PROP)
		{
			...
			ShowSyncHudText(client, g_Text4, "프롭 잠금"); //77
		}
	}
}
