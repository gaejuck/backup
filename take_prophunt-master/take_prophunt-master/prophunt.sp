...
public OnPluginStart()
{
	...
	RegAdminCmd("preload", preload, ADMFLAG_ROOT);
}

public Action:preload(client, args)
{
	for(new i = 1; i <= MaxClients; i++)
	{
		if(IsClientInGame(i) && IsPlayerAlive(i))
		{
			...
			if (!IsFakeClient(i)) SendConVarValue(i, g_hArenaRoundTime, "600");
			ResetPlayer(i);
		}
	}
	PrintToChat(client, "\x07FFFFFF리로드 완료되었습니다.");
...
public Action:TakeDamageHook(victim, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3], damagecustom)
{
	...
	if(victim > 0 && attacker > 0 && victim <= MaxClients && attacker <= MaxClients && IsClientInGame(victim) && IsClientInGame(attacker))
	{
		...
		else if(victim == attacker && TF2_GetPlayerClass(victim) == TFClassType:TFClass_Soldier) //솔저 데미지 수정 
		{
			damage = 45.0; // 90이 기본값
			return Plugin_Changed;
		}
		
		if(IsValidEntity(weapon)) //77 자라테
		{
...
public Action:TF2_CalcIsAttackCritical(client, weapon, String:weaponname[], &bool:result)
{
	...
	{
		if(strcmp(weaponname, "tf_weapon_flamethrower") == 0 || strcmp(weaponname, "tf_weapon_syringegun_medic") == 0) //77
		{
...
public OnGameFrame()
{
	...
	for (new client = 1; client <= MaxClients; client++)
	{
		...
		{
			DoSelfDamage(client, weapon);
			if(TF2_GetPlayerClass(client) != TFClassType:TFClass_Pyro) AddVelocity(client, 1.3);
			else AddVelocity(client, 1.0);
		}
	}
}
...
public WeaponSwitch(client, weapon)
{
	...
	if(strcmp(weaponname, "tf_weapon_flamethrower") != 0 || strcmp(weaponname, "tf_weapon_syringegun_medic") == 0)
	{
		g_CurrentlyFlaming[client] = false;
		g_FlameCount[client] = 0;
	}
		...
		if(IsPlayerAlive(client))
		{
			...
			if(GetClientTeam(client) == TEAM_PROP)
			{
				...
				if((buttons & IN_ATTACK2) == IN_ATTACK2 && !g_HoldingRMB[client])
				{
					...
					if(g_First[client])
					{
						PrintHintText(client, "3인칭 모드");
						SwitchView(client, true, false);
					}
					else
					{
						PrintHintText(client, "1인칭 모드");
	...
	if(!g_RoundOver)
	{
		...
		if(!g_LastProp && playas == 2 && GetClientTeam(client) == TEAM_PROP)
		{
			...
			for(new client2=1; client2 <= MaxClients; client2++)
			{
				...
				{
					if(GetClientTeam(client2) == TEAM_PROP) //77
					{
						g_LastPropPlayer = client2;
						TF2_RegeneratePlayer(client2);
						
						SetHudTextParams(-1.0, 0.22, 10.0, 255, 234, 255, 0, 2, 1.0, 0.05, 0.5);
						ShowHudText(client2, 1, "무기가 생겼습니다! 돌격하세요!");
						
						SetHudTextParams(-1.0, 0.27, 10.0, 249, 255, 61, 255, 2, 1.0, 0.05, 0.5);
						ShowHudText(client2, 2, "죽일 때마다 체력이 풀로 채워집니다.");
						
						SetHudTextParams(-1.0, 0.32, 10.0, 0, 204, 255, 255, 2, 1.0, 0.05, 0.5);
						ShowHudText(client2, 3, "근접무기는 황금렌치로 변경되며 데미지가 한방입니다.");
					...
					if(GetClientTeam(client2) == TEAM_HUNTER)
					{
						...
						SetHudTextParams(-1.0, 0.22, 10.0, 255, 234, 255, 0, 2, 1.0, 0.05, 0.5);
						ShowHudText(client2, 4, "레드팀 마지막 1인이 무기를 얻었습니다.");
						
						SetHudTextParams(-1.0, 0.27, 10.0, 249, 255, 61, 255, 2, 1.0, 0.05, 0.5);
						ShowHudText(client2, 5, "조심하세요!");
...
public Action:Timer_Locked(Handle:timer, any:entity)
{
	for(new client=1; client <= MaxClients; client++)
	{
		if(g_RotLocked[client] && IsClientInGame(client) && !IsFakeClient(client) && IsPlayerAlive(client) && GetClientTeam(client) == TEAM_PROP)
		{
			...
			ShowSyncHudText(client, g_Text4, "프롭 잠금");
		}
	}
}
