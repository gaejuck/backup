...
Handle:StopSound_cookie;
bool StopSound[MAXPLAYERS+1] = false;
...
public OnPluginStart(){
	...
	sm_sc_url = CreateConVar("sm_sc_url", "amtestwp.byethost10.com/soundcloud.php", "Soundcloud php script location. Needs to follow the default format", FCVAR_PLUGIN|FCVAR_PROTECTED);
	sm_sc_visible = CreateConVar("sm_sc_visible", "0", "1 = makes the window popup, 0 = plays in background", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	sm_sc_volume = CreateConVar("sm_sc_volume", "3", "default volume (0-10)", FCVAR_PLUGIN, true, 0.0, true, 10.0);
	
	RegConsoleCmd("sm_ccvvdwwaqzx", OnSc, "Play a song on soundcloud. Format: sm_sc song_name");
	...
	RegConsoleCmd("sm_scstopsound", OnStopSound, "stop sound");
	...
	StopSound_cookie = RegClientCookie("sm_soundcloud_stop", "soundcloud stop sound cookie", CookieAccess_Protected);
}

public OnClientConnected(client){
	volume[client] = GetConVarInt(sm_sc_volume);
	lastTime[client] = 0;
	StopSound[client] = false;
...
public OnClientCookiesCached(client){
	...
	char temp2[10];
	GetClientCookie(client, StopSound_cookie, temp2, sizeof(temp2));
	if(StrEqual(temp2, "Yes"))
		StopSound[client] = true;
	else
		StopSound[client] = false;
		
	if(StrEqual(temp2, "no"))
	{
		SetClientCookie(client, StopSound_cookie, "No");
	}
}
...
public Action:OnStopSound(int client, int args)
{
	if(StopSound[client] == false)
	{
		OnStop(client, 0);
		SetClientCookie(client, StopSound_cookie, "Yes");
		StopSound[client] = true;
		PrintToChat(client, "\x07FFFFFF이제부터 노래가 들리지 않습니다.");
	}
	else
	{
		SetClientCookie(client, StopSound_cookie, "No");
		StopSound[client] = false;
		PrintToChat(client, "\x07FFFFFF이제부터 노래가 들립니다.");
	}
	...
	if(args>0){
		...
		if(!StopSound[client])
		{
			...
			if(engineVersion == Engine_CSGO || engineVersion == Engine_Left4Dead || engineVersion == Engine_Left4Dead2){
				...
				PrintToChat(client, "%s[SoundCloud] %노랮 제목 : %s%s%s. 그외 다른 기능 %s!도움말%s을 쳐보세요!", cPRIMARY, cSECONDARY, cPRIMARY, arg, cSECONDARY, cPRIMARY, cSECONDARY);
			}
		}
	} else {
		PrintToChat(client, "!노래 '검색 할 노래'");
	}
	return Plugin_Handled;
}
...
public Action:OnStop(int client, int args){
	...
	if(engineVersion == Engine_CSGO || engineVersion == Engine_Left4Dead || engineVersion == Engine_Left4Dead2){
		...
		PrintToChat(client, "%s[SoundCloud] %s노래를 끕니다.", cPRIMARY, cSECONDARY);
	}
	return Plugin_Handled;
}
...
public Action:OnHelp(int client, int args){
	if(engineVersion == Engine_CSGO || engineVersion == Engine_Left4Dead || engineVersion == Engine_Left4Dead2){
		...
		PrintToChat(client, "%s[SoundCloud] %s원하는 노래를 검색할 수 있습니다. %s<!노래> \x03 | %s노래 목록을 보여줍니다. %s<!노래목록>", 
								cPRIMARY, cSECONDARY, cPRIMARY, cSECONDARY, cPRIMARY);
		PrintToChat(client, "%s[SoundCloud] %s볼륨을 설정할 수 있습니다. %s <!볼륨 0-10> \x03| %s노래를 끌 수 있습니다. %s<!스탑>",
		cPRIMARY, cSECONDARY, cPRIMARY, cSECONDARY, cPRIMARY);
		PrintToChat(client, "%s[SoundCloud] %s듣고 있는 노래의 제목을 볼 수 있습니다. %s<!제목>", cPRIMARY, cSECONDARY, cPRIMARY);
	}
	return Plugin_Handled;
}
public Action:OnOpen(int client, int args){
	if(lastTime[client]>0){
		...
		if(engineVersion == Engine_CSGO || engineVersion == Engine_Left4Dead || engineVersion == Engine_Left4Dead2){
			...
			PrintToChat(client, "%s[SoundCloud] %s노래를 듣고 있지 않습니다. (!노래 '들을 노래 검색')", cPRIMARY, cSECONDARY);
		}
	}
	return Plugin_Handled;
}
...
public Action:OnVolume(int client, int args){
	if(args==1){
		...
		if(chars>0 && vol<11){
			...
			if(engineVersion == Engine_CSGO || engineVersion == Engine_Left4Dead || engineVersion == Engine_Left4Dead2){
				...
				PrintToChat(client, "%s[SoundCloud] %s볼륨 설정 %s%d%s.", cPRIMARY, cSECONDARY, cPRIMARY, volume[client], cSECONDARY);
			}
	...
	PrintToChat(client, "!볼륨 < 1 ~ 10 > (현재 볼륨: %d)", volume[client]);
	return Plugin_Handled;
}
