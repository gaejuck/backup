...
public Action:CommandOctagon(client, args) return Plugin_Continue;
...
/*
**븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬븬
**     __  ___                                                  __ 
...
{    
    ...
    if (iIndex != -1)
    {
        ...
        if (iEntity && IsValidEntity(iEntity))
        {
			...
			SDKHook(iEntity, SDKHook_StartTouch, Event_StartTouch); //77
...
HomingRocketThink(iIndex)
{
    ...
    else if ((iDeflectionCount > g_iRocketDeflections[iIndex]))
    {
		...
		// PrintToChatAll("\x04이 로켓은 %N님 꺼다 | 타겟은 \x07FFFFFF%N", iClient, iTarget);
		
		if (iClient == iTarget) //77
		{
			if(GetClientTeam(iClient) == 2) iTarget = SelectTarget(3, iIndex);
			else if(GetClientTeam(iClient) == 3) iTarget = SelectTarget(2, iIndex);
		}
